...
exports.getPlaylistById = async (req, res) => {
  try {
    ...
    // Serialize translations Map to plain object
    const serializedWords = words.map(w => {
      const obj = w.toObject();
      if (w.translations instanceof Map) {
        obj.translations = Object.fromEntries(w.translations);
      }
      return obj;
    });
    res.json({ ...playlist.toObject(), words: serializedWords });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
